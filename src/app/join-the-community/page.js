...
export default function JoinCommunityPage() {
  ...
  useEffect(() => {
    ...
    if (user && !loading && !hasJoinedCommunity) {
      ...
      if (urlParams.get('joined') === 'true' || window.location.pathname === '/join-the-community') {
        setHasJoinedCommunity(true);
        // Clean up URL
        window.history.replaceState(null, '', '/join-the-community');
      }
    }
      ...
      <div className={`relative z-10 w-full ${contentHeight} flex flex-col items-center justify-start pt-32 lg:pt-40 px-4 ${verticalPadding}`}>
        ...
        >
          <span className="relative z-10 flex items-center gap-3">
            {hasJoinedCommunity ? 'Welcome to the Community!' : 'Join the Community Now'}
            {!hasJoinedCommunity && (
              <motion.span
                animate={{ x: [0, 5, 0] }}
