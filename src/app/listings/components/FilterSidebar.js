...
export default function FilterSidebar({ isOpen, onClose, className = "" }) {
  ...
  return (
    <>
      {/* Mobile Backdrop */}
      {isMobileOverlay && !isDesktopEmbedded && (
        <div 
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="mt-1">
                ...
                <div className="h-5 flex items-center">
                  <p className={`text-sm text-gray-500 dark:text-gray-400 ${activeFilterCount > 0 ? 'visible' : 'invisible'}`}> 
              ...
              <div className="w-10 h-10 flex items-center justify-center">
                {activeFilterCount > 0 && (
                  <div className="relative">
                    <span className="inline-flex items-center justify-center w-7 h-7 text-xs font-bold text-white bg-gradient-to-r from-primary-500 to-primary-600 rounded-full shadow-lg">
                      {activeFilterCount}
                    </span>
            ...
            <div className="flex items-center space-x-2">
              {activeFilterCount > 0 ? (
                <button
                  ...
                  className="px-3 py-1.5 text-sm text-primary-600 hover:text-white hover:bg-primary-600 font-medium transition-all duration-200 rounded-lg border border-primary-200 dark:border-primary-600 hover:border-primary-600"
                >
                  Clear All
                </button>
              ) : (
                <span className="px-3 py-1.5 text-sm invisible select-none">Clear All</span>
              )}
              {!isDesktopEmbedded && (
                <button
                  onClick={onClose}
    ...
    <div className="bg-white dark:bg-gray-800/50 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-600/70 hover:shadow-md transition-all duration-300">
      ...
      {isExpanded && (
        <div className="space-y-4 mt-4">
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                                 className="w-full pl-4 pr-10 py-3 text-sm border border-gray-200 dark:border-gray-500 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent focus:bg-white dark:focus:bg-gray-600 transition-all duration-200"
              />
          ...
          {searchable && filteredOptions.length === 0 && searchQuery && (
